...
<!-------------------------------------HOME------------------------------------->
		...
		include ('inc/termlinks.html');
			...
			if (isset($_GET['p']) && is_numeric($_GET['p'])) {
				...
				$q = "SELECT COUNT(term_id) FROM aw2274434_karate_entity_terms WHERE termtype_id = 3";
            ...
            $q="SELECT * FROM aw2274434_karate_entity_terms aw2274434_karate_enum_termtype WHERE `aw2274434_karate_enum_termtype`.`termtype_id` = 3 ORDER BY $order_by LIMIT $start, $display";
            $r = mysqli_query($dbc, $q);
            if (!$r) {
