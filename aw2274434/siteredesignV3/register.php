...
<!-------------------------------------Register------------------------------------->
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if 	(empty($trimmed['username'])){
						...
						if ($result = @mysqli_query($dbc, 'SELECT * FROM aw2274434_karate_users WHERE username = "'.$trimmed['username'].'"'))	{
					...
					if ($result = @mysqli_query($dbc, 'SELECT * FROM aw2274434_karate_users WHERE email = "'.$trimmed['email'].'"'))	{
							if (mysqli_num_rows($result) > 0){
					...
					if (empty($errors)) {
						...
						$q = "INSERT INTO aw2274434_karate_users (username, first_name, last_name, email, pass, reg_date) VALUES ('$un', '$fn', '$ln', '$e', SHA1('$p'), NOW() )";		
